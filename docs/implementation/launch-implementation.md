...
- `plan` / `planSchemaVersion` / `planPersistedAt`: secret-free authoritative Launch plan envelope. Written after `platform.plan` succeeds and before `platform.execute`. `planSchemaVersion` is the shell version (`"1"`). Envelope shape: `{ shellVersion, optionsOutcomes, platformPlan }` (`launchPlanEnvelopeV1Schema`). `optionsOutcomes` holds vanity reservation public fields and attribution policy. `platformPlan` is the pump.fun plan (`pumpfunLaunchPlanV1Schema`) with normalized money, public wallet identities, allocations, intended effects (bundle/Mayhem/distribution only), recovery caps, and opaque pump fields — never vanity/attribution flags or reservation ids, never private keys.
