...
6. Execute validates the persisted pump.fun plan (`pumpfunLaunchPlanV1Schema`) and branches:
   ...
   - **Bundled**: still the compat job (`runLaunchJob` / `createAndBuyInBundle`) until ticket 10. The bundled job hard-fails if invoked for a non-bundled launch.
...
8. **Create + Buy**: create token and execute creator/bundler buys.
   ...
   - Bundled launches still use the compat job + `createAndBuyInBundle` (Jito) until ticket 10 consolidates that path onto raw builders and removes remaining SDK usage.
