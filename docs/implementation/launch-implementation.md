...
4. Lifecycle resolves pump.fun via `resolveLaunchPlatform`, calls `platform.plan`, materializes Launch Options outcomes (vanity reserve when requested), composes options fees into normalized money, persists the plan envelope (`shellVersion` / `optionsOutcomes` / `platformPlan` on `plan` + `planSchemaVersion` / `planPersistedAt`), then calls `platform.execute` with that exact envelope on the lifecycle context.
...
- `launch.previewCosts` (query): accepts `versionedLaunchPreviewInputSchema` (Platform + Launch Options + config, no Token metadata). Routes through `resolveLaunchPlatform(...).preview`, composes vanity/attribution fees from Launch Options into normalized money, and returns the review envelope: `money` (`normalizedLaunchMoneySummarySchema` lamport strings + labeled line items) plus `mainWalletBalanceLamports`, `hasSufficientMainWallet`, `platformFeeWaived`, and `platformFeeDiscountRate`. Side-effect-free (read-only RPC/balance); does not persist plans, fund Wallets, publish metadata, or submit on-chain.
...
- `plan` / `planSchemaVersion` / `planPersistedAt`: secret-free authoritative Launch plan envelope. Written after `platform.plan` succeeds and before `platform.execute`. `planSchemaVersion` is the shell version (`"1"`). Envelope shape: `{ shellVersion, optionsOutcomes, money, platformPlan }` (`launchPlanEnvelopeV1Schema`). `money` is the lifecycle-composed summary (Platform money + Launch Options fees). `optionsOutcomes` holds vanity reservation public fields and attribution policy. `platformPlan` is opaque at the shell — pump.fun validates it with `pumpfunLaunchPlanV1Schema` on execute/recover (wallets, allocations, intended effects for bundle/Mayhem/distribution, recovery caps, opaque pump fields; never vanity/attribution flags or reservation ids; never private keys).
...
- `resolveLaunchPlatform`: typed registry resolves pump.fun only; unsupported Platforms throw before record creation. Each module exposes `preview` / `plan` / `execute` / `recover` / `compensatePlanResources`. pump.fun `preview` returns Platform money via the cost calculator then merges Launch Options fees; `plan` builds the secret-free pump `platformPlan` from versioned `config` (may create unfunded Wallet key refs; must not fund, submit on-chain, or own vanity reservation); `execute` validates the persisted envelope then routes non-bundled launches to `runPumpfunNonBundledExecute` and bundled launches to `runPumpfunBundledExecute`, returning typed outcomes; `recover` validates the envelope when present and reclaims SOL from persisted Managed Launch Wallet evidence with funded caps (`recoverSol` routes through it).
- Shared lifecycle (`launchLifecycle`): router entry for start/status/cancel/retry/getActive; owns Launch Options materialization (mint identity / vanity reserve), options fee composition, plan envelope durability (platform plan → options outcomes → persist envelope → execute); provides lifecycle contexts including the persisted envelope; maps typed Platform outcomes to `status` + `outcomeKind` / `outcomeDetails`; owns the post-success usage-fee helper. Start/cancel/retry orchestration helpers and Platform-owned execute job bodies still live in `launch.service.ts`. Fee collection runs only after Platform success and never downgrades a successful Launch when collection fails. Transitional lifecycle/execute compatibility delegates and the system creator Launch path are removed.
