...
6. **Fee collection**:
   - collect the bundled exit platform fee from the user's main wallet after all sell bundles land successfully
   - base bundled exit fee is `0.1 SOL`
   - `DEVELOPER` users receive the shared 25% platform-fee discount (`0.075 SOL`)
   - `PRO` users have the bundled exit platform fee waived
...
- Dialog discloses the bundled exit fee at the bottom of the Exit tab, including the user's active plan waiver/discount, and requires a final confirmation before starting
